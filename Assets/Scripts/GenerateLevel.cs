...
public class GenerateLevel : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        for (int i = 0; i < levelString.Length; i++) {
            ...
            else if (levelString[i] == 'l') {
                float yPosBackup = yPos;

                if (char.IsDigit(levelString[i + 1])) {
                    int digits = 1;
                    for (int j = 1; j < levelString.Length; j++){
                    ...
                    int loveHeight = int.Parse(levelString.Substring(i + 1, digits));
                    yPos += loveHeight;
                    i += digits;
                }

                spawnPos = new Vector3(xPos, yPos, 0);
                Instantiate(love, spawnPos, Quaternion.identity, level.transform);
